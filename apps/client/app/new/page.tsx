...
export default function NewTicketPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setStatus(error instanceof Error ? error.message : 'Failed to load ticket metadata.');
      }
    };
  ...
  const createTicket = async () => {
    ...
    const selectedEngineer = engineers.find((engineer) => engineer.id === form.engineer);

    setSubmitting(true);
    setStatus('');

    try {
      const response = await api<{ success?: boolean; error?: string }>('/api/v1/ticket/create', {
        method: 'POST',
        json: {
          name: form.name,
          title: form.title,
      ...
      setStatus(response.success ? 'Ticket created.' : response.error ?? 'Ticket creation failed.');
    } catch (error) {
      ...
      <Card className="max-w-4xl">
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Engineer">
            <select
              value={form.engineer}
              onChange={(event) => setForm((current) => ({ ...current, engineer: event.target.value }))}
              className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none"
            >
              <option value="">Unassigned</option>
